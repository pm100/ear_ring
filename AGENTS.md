...
```
All platforms use the same pre-rendered PNG (desktop/public/treble_clef.png, 299×638px,
transparent background). Generated via Windows GDI+ (StringFormat.GenericTypographic,
600px Segoe UI Symbol font) so it matches how Android's Skia engine renders the glyph.
...
(GDI+/Segoe UI Symbol, same as treble_clef.png) and distributed to all platforms.
...
```
Both scripts use PowerShell GDI+ (System.Drawing, Segoe UI Symbol font) internally.
