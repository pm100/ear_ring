...
export default function MusicStaff({ sequence, currentNoteIndex, highlightIndex, detected, status }: Props) {
  ...
  const clefFontSize = 56;
  const clefX = 4;
  const clefY = staffTop - lineSpacing * 1.5;
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${svgWidth} ${svgHeight}`} style={{ display: 'block' }}>
      ...
      <text x={clefX} y={clefY} fontSize={clefFontSize} fontFamily="serif" fill="#333" style={{ userSelect: 'none' }}>{'\uD834\uDD1E'}</text>
